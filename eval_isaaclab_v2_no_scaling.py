# eval_isaaclab_v2_no_scaling.py
# Like eval_isaaclab_v2.py but with no observation/action scaling, no offset, no normalization.
# Raw IsaacLab observations are passed directly to the policy, and raw policy
# outputs are passed directly to env.step() without any conversion.
import sys
import os
...
class OnlineEvalNoScaling:
    def __init__(
        self,
        task_name,
        seed,
        dataset_path="offline_dataset_no_scaling.hdf5",
        ...
        self.dataset = load_hdf5_dataset(dataset_path)
        ...
        # Override action scale to 1 so raw absolute joint positions are applied directly
        env_cfg.actions.joint_pos.scale = 1.0
        env_cfg.actions.joint_pos.offset = 0.0
    ...
    def calculate_total_reward(self, rewbuffer, ep_infos, lenbuffer):
        ...
        if ep_infos:
            for key in ep_infos[0].keys():
                ...
                scaled_rew_terms_avg[key] = float(np.mean(infotensor))
