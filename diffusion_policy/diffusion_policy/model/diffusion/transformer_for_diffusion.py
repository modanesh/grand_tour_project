...
class TransformerForDiffusionInferenceCassie(ModuleAttrMixin):
    def __init__(self,
            input_dim: int,
            output_dim: int,
            horizon: int,
    ...
    def forward(self, 
        ...
        cond_obs_emb_2 = self.cond_obs_emb_2(cond[...,-5:])
        # (B,To,n_emb)
        cond_embeddings_cat = torch.cat([cond_embeddings, cond_obs_emb, cond_obs_emb_2], dim=1)
        tc = cond_embeddings_cat.shape[1]
        position_embeddings = self.cond_pos_emb[
            :, :tc, :
        ]  # each position maps to a (learnable) vector
        x = self.drop(cond_embeddings_cat + position_embeddings)
        x = self.encoder(x)
        memory = x
        # (B,T_cond,n_emb)
...
class TransformerForDiffusionInferenceGo1(ModuleAttrMixin):
    def __init__(self,
            input_dim: int,
            output_dim: int,
            horizon: int,
